...
public class UserRequestController extends HttpServlet {
	...
	/*
	 try {
	    ...
	    e1.printStackTrace();
	    error.append("Неправильный формат Email \n");
	} catch (LoginIncorrectException e2) {
	    error.append("Неправильный формат Login \n");
	    e2.printStackTrace();
	}*/
	...
	try {
	  //  ud.insert(user);
	   ...
	   // urd.insert(ureq);
	    ...
	    error.append("Произошла ошибка При работе с базой данных \n");
	    /*
	     * PrintWriter pw = resp.getWriter(); pw.write(
	     * "<html><head><title>ERROR, DataBase not not allowed!</title></head><body>Ошибка! База данных не доступна!</body></html>"
	     * ); pw.flush(); pw.close();
	     */
	}
